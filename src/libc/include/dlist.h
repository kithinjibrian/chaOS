...
#define dlist_prev_entry(pos, member) \
	dlist_entry((pos)->member.prev, ____typeof____(*(pos)), member)
 ...
 */
static inline void __dlist_add__(dlist_t *new, dlist_t *prev, dlist_t *next)
{
	new->next = next;
	new->prev = prev;
	prev->next = new;
...
{
	__dlist_add__(new, head->prev, head);
}
